        ...
        void OnGUI()
        {
            ...
            if (!manager.IsClientConnected() && !NetworkServer.active && manager.matchMaker == null)
            {
                if (noConnection)
                {
                    ...
                    if (GUI.Button(new Rect(xpos, ypos, 105, 200), "LAN Client(C)"))
                    {
                        manager.StartClient();
                    }

                    manager.networkAddress = GUI.TextField(new Rect(xpos + 100, ypos, 95, 200), manager.networkAddress);
                    ypos += spacing;
                    ...
                    {
                        if (GUI.Button(new Rect(xpos, ypos, 200, 200), "LAN Server Only(S)"))
                        {
                            manager.StartServer();
                        }
                        ypos += spacing;
